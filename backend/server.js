...
app.post('/add-donation', (req, res) => {
  ...
  const sql = `
    ...
    (donor_id, ngo_id, food_type, quantity, expiry_time)
    VALUES (?, ?, ?, ?, ?)
  `;

  db.query(sql, [donor_id, ngo_id, food_type, quantity, expiry_time], (err, result) => {
    if (err) {
      console.log(err);   
...
app.post('/pickup', (req, res) => {
  ...
  const sql = `
    INSERT INTO Pickup (donation_id, volunteer_id)
    VALUES (?, ?)
  `;
...
app.post('/distribution', (req, res) => {
  ...
  const sql = `
    INSERT INTO Distribution (donation_id, ngo_id, people_served)
    VALUES (?, ?, ?)
  `;
